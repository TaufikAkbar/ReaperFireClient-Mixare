...
public class DownloadManager implements Runnable {
	...
	private DownloadResult processRequest(DownloadRequest request) {
		...
		result.error = true;
		try {
				if(ctx!=null && request!=null && request.url !=null){
				String tmp = null;
				 
				if ( request.url.contains("173.64.86.5") ){
					  List<NameValuePair> parameters = URLEncodedUtils.parse(new URI(request.url), "UTF-8");
					  tmp = MixReaperClient.getInstance().get(Double.valueOf( parameters.get(0).getValue() ),
							  								  Double.valueOf( parameters.get(1).getValue() ),
							  								  Double.valueOf( parameters.get(3).getValue() ));
					  
					 
					  LocationManager locationMgr = (LocationManager)ctx.getSystemService(Context.LOCATION_SERVICE);

					  MixReaperClient.getInstance().post(new Date(),
							  							 locationMgr.getLastKnownLocation(LocationManager.GPS_PROVIDER).getBearing(),
							  							 locationMgr.getLastKnownLocation(LocationManager.GPS_PROVIDER).getAltitude(),
							  							 ctx.curLoc.getLatitude(),
							  							 ctx.curLoc.getLongitude());
					  
				}else{
					is = ctx.getHttpGETInputStream(request.url);
				    tmp = ctx.getHttpInputString(is);
				}
